qx.Class.define("sical3.comp.windowEspacios",
{
	...
	var functionActualizar = function(id_espacio) {
		var p = {};
		...
		rpc.addListener("completed", function(e){
			...
			if (id_espacio != null) {
				tbl.blur();
				tbl.buscar("id_espacio", id_espacio);
				tbl.focus();
			}
		});
		rpc.callAsyncListeners(true, "autocompletarEspacio", p);
	...
	commandAgregar.addListener("execute", function(e){
		var win = new sical3.comp.windowEspacio();
	...
	commandEditar.addListener("execute", function(e){
		...
		var win = new sical3.comp.windowEspacio(rowData);
		win.addListener("aceptado", function(e){
			var data = e.getData();
	...
	tableModel.setColumns(["Código", "Descripción"], ["codigo", "denominacion"]);
